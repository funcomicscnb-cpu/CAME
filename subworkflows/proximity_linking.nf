process PROXIMITY_LINKING_PLACEHOLDER {
    ...
    case "${re_to_gene_inference_stub}" in
      true|TRUE|1|yes|YES)
        ...
        ;;
      *)
        echo "ERROR: re_to_gene_inference real-mode proximity linking is scaffold-only in Stage 20; no production links are emitted." >&2
        exit 1
        ;;
    esac
    """
...
workflow PROXIMITY_LINKING {
    take:
    prepared_manifest
    re_to_gene_inference_stub

    main:
    PROXIMITY_LINKING_PLACEHOLDER(prepared_manifest, re_to_gene_inference_stub)

    emit:
    links = PROXIMITY_LINKING_PLACEHOLDER.out.links
    warnings = PROXIMITY_LINKING_PLACEHOLDER.out.warnings
}
