process PROMOTER_LINKING_PLACEHOLDER {
    ...
    case "${re_to_gene_inference_stub}" in
      true|TRUE|1|yes|YES)
        ...
        ;;
      *)
        echo "ERROR: re_to_gene_inference real-mode promoter linking is scaffold-only in Stage 20; no production links are emitted." >&2
        exit 1
        ;;
    esac
    """
...
workflow PROMOTER_LINKING {
    take:
    prepared_manifest
    re_to_gene_inference_stub

    main:
    PROMOTER_LINKING_PLACEHOLDER(prepared_manifest, re_to_gene_inference_stub)

    emit:
    links = PROMOTER_LINKING_PLACEHOLDER.out.links
    warnings = PROMOTER_LINKING_PLACEHOLDER.out.warnings
}
