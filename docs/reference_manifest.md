...
`reference_manifest.tsv` remains backward compatible with Stage 1-22. Keep legacy columns such as `genome_fasta`, `gtf`, `transcript_fasta`, `star_index`, `bwa_index`, `chrom_sizes`, `annotation_version`, and `source`; add the Stage 23 real-mode columns alongside them.

## Required Real-Mode Columns

The Stage 23 validator requires one row per reference with:
...
Legacy Stage 1-22 consumers read:
...
- `bwa_index`: used as the WGS BWA-MEM2 index prefix when `bwa_index_prefix` is absent, and retained for legacy scaffold compatibility.

Do not remove these columns from shared examples until downstream Stage 1-22 tools have been migrated.
...
Assay declarations add targeted warnings. RNA expects `transcript_fasta` and `star_index`. ATAC expects `bowtie2_index`, `chrom_sizes`, `blacklist_bed`, and `mappability_bed`; Stage 24 ATAC real mode uses Bowtie2 plus MACS3. Stage 27 WGS expects `fasta`, `fai`, `dict`, and `bwa_index` or `bwa_index_prefix` unless the FASTA is available for BWA-MEM2 index building. WES production support is not implemented.

Paths are metadata by default. Use `--check-paths` only when the validator should require local path existence.

## Stage 26 Reference Quality

Stage 26 adds content-level checks without changing the Stage 23 manifest schema. It validates declared reference assets, parses assembly reports into sequence-name alias maps, and checks FASTA/FAI/annotation sequence-name concordance.

Accepted Stage 26 aliases:
