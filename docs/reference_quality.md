...
Reference-quality validation checks whether declared reference assets are internally consistent before expensive RNA-seq, ATAC-seq, or optional WGS work starts.

This workflow validates readiness. It does not build references, run BUSCO, run RepeatMasker, run GenMap, certify a reference for production, or run WGS variant calling.
...
The most common failure is a sequence-name mismatch, such as FASTA names `1` and `MT` with annotations using `chr1` and `chrM`. CAME detects these cases and reports a direct alias-map or normalization fix instead of letting the mismatch surface later as empty counts or missing QC metrics.
...
When `--enable_real_mode_validation true` and `--omics_mode real` are used with `--run_stage bulk_omics`, reference-quality validation runs as an optional preflight after metadata/reference manifest checks and before RNA/ATAC work. The same validation gate runs for `--run_stage wgs_variants --wgs_mode real`, using `--reference_quality_assay wgs`.

Stub mode does not run the reference-quality preflight.

## Required And Recommended Assets

CAME keeps CAME manifest semantics unchanged. It uses existing columns first and accepts common aliases:
...
- ATAC: `fasta` and `fai`. `bowtie2_index` is optional when FASTA is buildable; `tss_bed` is recommended when TSS enrichment should be reported.
- WGS reference checks: `fasta`, `fai`, `dict`, and either `bwa_index`/`bwa_index_prefix` or buildable FASTA. These checks support CAME WGS SNP/indel calling but do not perform variant calling themselves.
...
CAME validates manifest and metadata schema, runs baseline RNA/ATAC real-mode workflows, and can run the optional WGS SNP/indel baseline. Reference-quality validation sits before those real-mode workflows when explicitly enabled: it checks reference content consistency and readiness without changing sample metadata contracts or downstream RNA/ATAC/WGS output paths.

## CAME Fixtures

The CAME fixture reference under `assets/test_data/real_mode_fixtures/tiny_reference/` includes a tiny FASTA, matching FAI, sequence dictionary, GTF, GFF3, assembly report, alias map, chrom sizes, and small BED helper files. It is designed so `--run_stage reference_quality --check_paths true` can validate reference contracts without downloading public data.
